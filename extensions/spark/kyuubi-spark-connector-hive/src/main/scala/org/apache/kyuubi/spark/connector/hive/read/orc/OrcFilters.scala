...
object OrcFilters extends OrcFiltersBase {
  ...
  def createFilter(
      schema: StructType,
      filters: Seq[Filter],
      isCaseSensitive: Boolean): Option[SearchArgument] = {
    val dataTypeMap = OrcFilters.getSearchableTypeMap(schema, isCaseSensitive)
  ...
  def getPredicateLeafType(dataType: DataType): PredicateLeaf.Type = dataType match {
    ...
    case ByteType | ShortType | IntegerType | LongType |
        TimestampNTZType => PredicateLeaf.Type.LONG
      ...
      case other =>
        buildLeafSearchArgument(dataTypeMap, other, builder).getOrElse {
          throw new SparkException(
            ...
            messageParameters = Map("owner" -> "OrcFilters.buildSearchArgument"),
            cause = null)
        }
    }
  }
