...
abstract class AbstractHivePartitionReaderFactory(
    sqlConf: SQLConf,
  ...
  override def createReader(partition: InputPartition): PartitionReader[InternalRow] = {
    ...
    val iter: Iterator[HivePartitionedFileReader[InternalRow]] =
      filePartition.files.toIterator.map { file =>
        ...
        setPushFilters(readDataSchema, pushedFilters, isCaseSensitive, broadcastHiveConf)
